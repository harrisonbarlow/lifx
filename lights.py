...
def get_light (name=None):
    num_lights = 3 # Faster discovery, when specified
...
def update_light ():
    global bulb, bbox, timer, interval, last_colour
    ...
    img = np.array(img)
    [r, g, b, a] = np.mean(img, axis=tuple(range(img.ndim-1)))
